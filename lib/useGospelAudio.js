...
export function useGospelAudio(chords = [], genre = 'Contemporary', lyrics = []) {
  ...
  const [backingTrackLoading, setBackingTrackLoading] = useState(false);
  const [playbackMode, setPlaybackModeState] = useState("suno"); // "suno" | "harmony"
  const playbackModeRef = useRef("suno");
  ...
  // References for vocal stems AudioBuffers and active Web Audio play nodes
  const stemBuffersRef = useRef({ soprano: null, alto: null, tenor: null, lead: null });
  const stemSourcesRef = useRef({ soprano: null, alto: null, tenor: null, lead: null });
  const stemGainsRef = useRef({ soprano: null, alto: null, tenor: null, lead: null });
  const stemStatesRef = useRef({
    lead:    { vol: 90, muted: false, solo: false },
    soprano: { vol: 85, muted: false, solo: false },
    alto:    { vol: 80, muted: false, solo: false },
    tenor:   { vol: 75, muted: false, solo: false },
  });

  const backingTrackBufferRef = useRef(null);
  const backingTrackSourceRef = useRef(null);
  ...
  // Helper to load and decode vocal WAV files into AudioBuffers
  const loadStems = useCallback(async (urls) => {
    if (!urls) return;
    setStemsLoading(true);
    setStemsLoaded(false);
    const ctx = getCtx();
    try {
      const promises = Object.entries(urls).map(async ([voice, url]) => {
        if (!url) return;
        const res = await fetch(url);
        if (!res.ok) throw new Error(`Failed to fetch stem for ${voice}`);
        const arrayBuffer = await res.arrayBuffer();
        const audioBuffer = await new Promise((resolve, reject) => {
          ctx.decodeAudioData(arrayBuffer, resolve, reject);
        });
        stemBuffersRef.current[voice] = audioBuffer;
      });
      await Promise.all(promises);
      setStemsLoaded(true);
    } catch (e) {
      console.error("Error loading voice stems into AudioContext:", e);
    } finally {
      setStemsLoading(false);
    }
  }, []);

  // Helper to load and decode backing track WAV files into AudioBuffer
  const loadBackingTrack = useCallback(async (url) => {
    if (!url) return;
    setBackingTrackLoading(true);
    setBackingTrackLoaded(false);
    const ctx = getCtx();
    try {
      const res = await fetch(url);
      if (!res.ok) throw new Error(`Failed to fetch backing track from ${url}`);
      const arrayBuffer = await res.arrayBuffer();
      const audioBuffer = await new Promise((resolve, reject) => {
        ctx.decodeAudioData(arrayBuffer, resolve, reject);
      });
      backingTrackBufferRef.current = audioBuffer;
      setBackingTrackLoaded(true);
    } catch (e) {
      console.error("Error loading backing track into AudioContext:", e);
    } finally {
      setBackingTrackLoading(false);
    }
  }, []);

  const clearBackingTrack = useCallback(() => {
    backingTrackBufferRef.current = null;
    setBackingTrackLoaded(false);
  }, []);

  // Stops and clears all active vocal stem buffer source nodes
  const stopStems = useCallback(() => {
    Object.keys(stemSourcesRef.current).forEach(voice => {
      const source = stemSourcesRef.current[voice];
      if (source) {
        try { source.stop(); } catch(e){}
        stemSourcesRef.current[voice] = null;
      }
      stemGainsRef.current[voice] = null;
    });
  ...
  // Stops active stem sources and wipes their buffers so a Suno full-mix
  // doesn't get doubled by the local SATB choir voices.
  const clearStems = useCallback(() => {
    stopStems();
    Object.keys(stemBuffersRef.current).forEach(v => { stemBuffersRef.current[v] = null; });
    setStemsLoaded(false);
  }, [stopStems]);

  // Recalculates and sets gain values dynamically based on vol, mute, and solo states
  const applyStemGains = useCallback((states) => {
    if (!states) return;
    const hasSolo = Object.values(states).some(s => s.solo);
    Object.entries(states).forEach(([voice, s]) => {
      stemStatesRef.current[voice] = { vol: s.vol, muted: s.muted, solo: s.solo };
      
      const gainNode = stemGainsRef.current[voice];
      if (gainNode && ctxRef.current) {
        const isMuted = s.muted || (hasSolo && !s.solo);
        const targetVol = isMuted ? 0 : (s.vol / 100);
        gainNode.gain.setValueAtTime(targetVol, ctxRef.current.currentTime);
      }
    });
  }, []);
  ...
  const runScheduler = useCallback(() => {
    ...
    const isBackingActive = playbackModeRef.current === "suno" && backingTrackBufferRef.current;
    const destinations = {
      piano: isBackingActive ? null : pianoGainRef.current,
      percussion: isBackingActive ? null : percussionGainRef.current,
      bass: isBackingActive ? null : bassGainRef.current,
      guitar: isBackingActive ? null : guitarGainRef.current
    };
  ...
  const play = useCallback(() => {
    ...
    const barDur = spb * 4;
    const offset = chordIdxRef.current * barDur;

    const isBackingActive = playbackModeRef.current === "suno" && backingTrackBufferRef.current;

    // Start custom backing track if loaded
    if (isBackingActive) {
      if (backingTrackSourceRef.current) {
        try { backingTrackSourceRef.current.stop(); } catch(e){}
      }
      const source = ctx.createBufferSource();
      source.buffer = backingTrackBufferRef.current;
      source.loop = true;
      // Connect to pianoGainRef.current to bind master backing track volume to the "piano" volume slider
      source.connect(pianoGainRef.current);
      
      let backingOffset = offset;
      if (backingTrackBufferRef.current.duration > 0) {
        backingOffset = backingOffset % backingTrackBufferRef.current.duration;
      }
      source.start(startTime, backingOffset);
      backingTrackSourceRef.current = source;
    }

    // Start all loaded vocal stem buffer sources in perfect, sample-accurate sync
    if (!isBackingActive) {
      Object.keys(stemBuffersRef.current).forEach(voice => {
        const buffer = stemBuffersRef.current[voice];
        if (!buffer) return;

        if (stemSourcesRef.current[voice]) {
          try { stemSourcesRef.current[voice].stop(); } catch(e){}
        }
        ...
        const s = stemStatesRef.current[voice];
        const hasSolo = Object.values(stemStatesRef.current).some(vs => vs.solo);
        const isMuted = s.muted || (hasSolo && !s.solo);
        gain.gain.value = isMuted ? 0 : (s.vol / 100);

        source.connect(gain);
        gain.connect(ctx.destination);

        let stemOffset = offset;
        if (buffer.duration > 0) {
          stemOffset = stemOffset % buffer.duration;
        }
        source.start(startTime, stemOffset);
        stemSourcesRef.current[voice] = source;
        stemGainsRef.current[voice] = gain;
      });
    }
  ...
  const pause = useCallback(() => {
    ...
    stopStems();
    // Close context to immediately silence all scheduled-but-not-yet-played nodes
    if (ctxRef.current) {
      ctxRef.current.close().catch(() => {});
      ctxRef.current = null;
      pianoGainRef.current = null;
      percussionGainRef.current = null;
  ...
  // Restart playback if backing track or stems finish loading during active play to prevent overlap/desync
  useEffect(() => {
    if (isPlayingRef.current) {
      restartPlayback();
    }
  }, [backingTrackLoaded, stemsLoaded, restartPlayback]);
