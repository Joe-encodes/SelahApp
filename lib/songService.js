...
export async function getSong(localId) {
  let local = await idbGetSong(localId);
  if (!local && localId !== undefined && localId !== null) {
    const fallbackId = typeof localId === "number" ? String(localId) : Number(localId);
    if (typeof fallbackId === "string" || !isNaN(fallbackId)) {
      local = await idbGetSong(fallbackId);
    }
  }
